...
## Executable workflow replay (Issue #294 Phase 2)

Two of the eleven canonical workflows are generated and checked by the
...
outcome update, and cohort summary. Both use bundled fictional inputs, pass
state only through declared artifact bundles, require explicit offline JSON
inputs, and remove API-key, token, secret, password, and proxy variables from
the subprocess environment. No replay command supplies `--symbols`,
`--fmp-universe`, or `--api-key`. This is an `offline-input-required` policy,
not an operating-system network sandbox.

Each full path ends with a `manual_contract`: it records a supplied human
decision bound by SHA-256 to the immediately preceding cohort evidence, but
does not claim to execute or validate a native trader-memory-core journal
append API. Decision gates in both replays are `record_only`, matching their
...
freezes the other nine current workflows as Phase 2 deferrals linked to Issue
