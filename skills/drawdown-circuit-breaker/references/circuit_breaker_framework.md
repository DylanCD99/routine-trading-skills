...
`PARTIAL` fails closed with `HALTED` and an `incomplete_state_data` rule whenever a thesis, ledger entry, or terminal result is skipped, malformed, non-finite, missing, or conflicts with another P&L source. Its `active_until` is `null` because release requires state repair and a rerun, not elapsed time. The sole recoverable warning is a finite `outcome.pnl_dollars` value used for an otherwise well-formed legacy terminal thesis that has no realized-P&L ledger entry; malformed or ledger-shaped history never takes this fallback. The recoverable fallback remains `PARTIAL` for audit visibility but does not by itself change the calculated recommendation.
